...
path find_path(const string& source,const string& target,imdb & db){
    list<path> partialPaths;
    set<string> previouslySeenActors;
    set<film>  previouslySeenFilms;
    path cur(source);
    partialPaths.push_back(cur);
    ...
    while(!partialPaths.empty()&&partialPaths.front().getLength()<6){
      ...
      vector<film> films;
      const string& lastActor=current.getLastPlayer();
            ...
            db.getCast(films[i],players);
              for(int j=0;j<players.size();j++){
                if(previouslySeenActors.find(players[j])==previouslySeenActors.end()){
                  ...
                  current.addConnection(films[i],players[j]);
                  if(players[j]==target){
                    return current;
                  }
                  partialPaths.push_back(current);;
                  current.undoConnection();
                }
              }
          }
        }
...
int main(int argc, const char *argv[])
{
  ...
  while (true) {
    ...
    if (source == target) {
      ...
      const path answer=find_path(source,target,db);
      if(answer.getLength()){
          cout<<answer;
      }else{
