...
(define (partition pivot num-list cmpFunction)
  (if (null? num-list) '(() ())
	...
	(if (cmpFunction (car num-list) pivot)
...
;This function gets list of circles and return list of all intersections of every pair.

(define (intersection-points circles)
  (if (or (null? circles)(null? (cdr circles))) '()
	  (append (apply append 
			 (map (lambda (value) (intersect (car circles) value)) 
			      (cdr circles)))
		  (intersection-points (cdr circles)))))


;This function get one point and list of  points and return the product of the distances
; between this point and every other one in list, if it is in the list it will be removed.
(define (distance-product point pointList)
  (apply * (map (lambda (pt) (dist point pt)) 
    (remove point pointList)))
)


;This function get list of points and rates them with apporpriate to their distance products.
(define (rate-points pointList)
  (map (lambda (value) (list (distance-product value pointList) value)) pointList)
)



;This function gets point lists and sorts them using quicksort,
;after this function I have modified quicksort implementation.
(define (sort-points pointList)
  (quicksort pointList (lambda (first second) (< (car first) (car second))))
)

;This function gets the list of points then, rate them,after it sort and the returns
;half of them.
(define (clumped-points pointList)
  (apply append(map  cdr (prefix-of-list (sort-points (rate-points pointList))
                                                      (quotient (length pointList) 2))))
)

;This function get list of points and returns an average point with distance-product.
(define (average-point pointList)
  (let ((avg-point
        (list (/ (apply + (map radius pointList)) (length pointList))
              (/ (apply + (map center pointList))  (length pointList)))))
        (list (distance-product avg-point pointList) avg-point))
)

;This function gets the list of points and returns the average of clumped points.
(define (best-estimate pointList)
  (average-point (clumped-points (intersection-points pointList)))
)

;THis function gets distances and star points and best-estimates all-guesses and then sorts them.
